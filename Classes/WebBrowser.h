...
public:
	/// 构造函数
	WebBrowser(Widget* pParent);
	virtual ~WebBrowser();

	LPCTSTR GetHomePage();
	bool IsAutoNavigation();
	void SetBkColor(COLORREF clr);
	void SetLoadingImage(CxImage* pImage);
	void SetShowLoading(BOOL bShow);
	...
	static boost::signal<bool (LPCTSTR, WebBrowser *)> &sigProcessUrl() { return m_sigProcessUrl;}
	boost::signal<void (Size, uint32_t, uint32_t, bool, bool)> &sigCustomWindow() { return m_sigCustomWindow;}
...
protected:
	...
	virtual HRESULT RegisterEventHandler(BOOL inAdvise);
	virtual void SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue);
	void CheckCustomWnd();
	std::string GetMetaContent(LPCTSTR lpszName);
...
protected:
	...
	DWORD m_dwCookie;
	std::string m_strHomePage;		// 默认页面
	bool m_bAutoNavi;				// 是否启动时打开默认页面

	BOOL m_bShowLoading;
	BOOL m_bLoading;
	COLORREF m_clrBk;
	CxImage* m_pLoadingImage;
	long m_lFrame;
 	Timer m_Timer;


private:
	static boost::signal<bool (LPCTSTR, WebBrowser*)> m_sigProcessUrl;
	boost::signal<void (Size, uint32_t, uint32_t, bool, bool)> m_sigCustomWindow;
};
